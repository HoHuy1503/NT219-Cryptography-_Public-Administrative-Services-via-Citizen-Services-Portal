...
KC = 'http://localhost:8080/realms/govportal/protocol/openid-connect/token'
 
def login(username, password):
    r = requests.post(KC, data={
        'grant_type': 'password',
        'client_id': 'citizens-app',
        'username': username,
        'password': password
    })
...
success = sum(login('citizen001', 'Test@1234') for _ in range(100))
success_rate = success / 100 * 100
print(f'Success rate: {success_rate:.1f}% (threshold ≥99%)')
 
# 50 login sai credential → false-accept phải = 0
false_accepts = sum(login('citizen001', f'WrongPass_{i}') for i in range(50))
print(f'False-accept: {false_accepts} (threshold = 0)')
 
# Test lockout: 5 sai liên tiếp
for i in range(6):
    r = requests.post(KC, data={
        'grant_type':'password','client_id':'citizens-app',
        'username':'citizen002','password':'WRONGWRONGWRONG'
    })
    if i == 5:
