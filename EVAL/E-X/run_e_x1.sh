...
OLD=$(vault read -field=latest_version transit/keys/falcon-doc-signing)
echo "Phiên bản hiện tại: $OLD"
START=$(date +%s)
 
vault write -f transit/keys/falcon-doc-signing/rotate
 
NEW=$(vault read -field=latest_version transit/keys/falcon-doc-signing)
END=$(date +%s)
ELAPSED=$((END-START))
...
vault read -field=min_decryption_version transit/keys/falcon-doc-signing
