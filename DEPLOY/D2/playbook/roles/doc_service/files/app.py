...
import base64, hashlib, logging, os, sys, time, uuid, requests, json
...
def add_cors_headers(response):
  ...
  # allow custom portal headers (X-User-ID, X-User-Type) and common request headers
  response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, X-Requested-With, X-User-ID, X-User-Type"
...
STATE_DIR = os.environ.get("DOC_STATE_DIR", "/state")
PKI_CA_KEY = os.path.join(STATE_DIR, "pki_ca_key.pem")
PKI_CA_CERT = os.path.join(STATE_DIR, "pki_ca_cert.pem")
PKI_CERT_STORE = os.path.join(STATE_DIR, "pki_issued_certs.json")
...
FILE_PUBLIC_KEY_PATH = os.path.join(STATE_DIR, "ml_pub_published.json")
...
ML_PRIV = os.path.join(STATE_DIR, "ml_priv.bin")
ML_PUB = os.path.join(STATE_DIR, "ml_pub.bin")
...
def _load_or_create_ca():
  if os.path.exists(PKI_CA_KEY) and os.path.exists(PKI_CA_CERT):
    ca_key = serialization.load_pem_private_key(_read_text(PKI_CA_KEY).encode('utf-8'), password=None)
    ca_cert = x509.load_pem_x509_certificate(_read_text(PKI_CA_CERT).encode('utf-8'))
...
def generate_ml_keypair():
  ...
  with open(ML_PRIV, 'wb') as f:
    f.write(priv_pem)
  with open(ML_PUB, 'wb') as f:
    f.write(pub_pem)
  return pub_pem, priv_pem

def load_or_create_ml_keys():
  if os.path.exists(ML_PRIV) and os.path.exists(ML_PUB):
    with open(ML_PRIV, 'rb') as f:
      sk = f.read()
